...
def read_fastfood_catalog(filename, dtype=None):
    """
    ...
    dtype: numpy dtype for returned arrays. Default ``numpy.float``
    ...
    X, Y, Z, W: numpy arrays
        Returns the triplet of X/Y/Z positions and weights as separate numpy arrays.
    ...
    with open(filename, "rb") as f:
        ...
        for field in 'xyzw':
            ...
            skip2 = struct.unpack(bytes_to_native_str(b'@i'), f.read(4))[0]
            pos[field] = array.astype(dtype)

    return [np.array(pos[name]) for name in 'xyzw']


def read_ascii_catalog(filename, dtype=None):
    """
    ...
    if pd is not None:
        df = pd.read_csv(filename, header=None,
                         engine='c',
                         dtype={name: dtype for name in 'xyzw'},
                         delim_whitespace=True)
        xyzw = np.asarray(df, dtype=dtype).T
    else:
        xyzw = np.genfromtxt(filename, dtype=dtype, unpack=True)

    return xyzw


def read_catalog(filename, dtype=np.float64):
    """
    ...
    ``x y z w`` - Unpacked numpy arrays compatible with the installed
    version of ``Corrfunc``.
    ...
    if file_exists(filename):
        ...
        xyzw = f(filename, dtype)
        return xyzw

    raise IOError('Could not locate file {0}'.format(filename))
